...
namespace Abc.Zebus.Tests.Transport
{
    ...
    public class TransportMessageWriterTests
    {
        ...
        public void should_serialize_transport_message()
        {
            ...
            var deserializedTransportMessage = Serializer.Deserialize<TransportMessage2>(new MemoryStream(stream.Buffer, 0, stream.Position));
            ...
            deserializedTransportMessage.Content.ShouldEqualDeeply(contentBytes);
            deserializedTransportMessage.Originator.InitiatorUserName.ShouldEqual(transportMessage.Originator.InitiatorUserName);
            deserializedTransportMessage.Environment.ShouldEqual(transportMessage.Environment);
            deserializedTransportMessage.WasPersisted.ShouldEqual(true);
        }
